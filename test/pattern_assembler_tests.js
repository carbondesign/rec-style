  ...
  var path = require('path');
	...
	exports['pattern_assembler'] = {
		'find_pattern_partials finds partials' : function(test){
			...
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment(description: 'A life is like a garden. Perfect moments can be had, but not preserved, except in memory.') }}</div>",
			};
			...
			test.equals(results[0], '{{> components-comment-header}}');
			test.equals(results[1], '{{> components-single-comment(description: \'A life is like a garden. Perfect moments can be had, but not preserved, except in memory.\') }}');
		...
		'find_pattern_partials_with_style_modifiers finds style modifiers' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment:foo }}</div>",
			};
			...
			test.equals(results[0], '{{> components-single-comment:foo }}');
		...
		'find_pattern_partials_with_style_modifiers finds style modifiers with parameters present too' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment:foo(bar:'baz') }}</div>",
			};
			...
			var results = pattern_assembler.find_pattern_partials_with_style_modifiers(currentPattern);
			test.equals(results.length, 1);
			test.equals(results[0], "{{> components-single-comment:foo(bar:'baz') }}");
		...
		'find_pattern_partials_with_style_modifiers finds style modifiers with verbose partials' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> 01-molecules/06-components/components-comment-header}}</h1><div>{{> 01-molecules/06-components/components-single-comment:foo }}</div>",
			};
			...
			var results = pattern_assembler.find_pattern_partials_with_style_modifiers(currentPattern);
			test.equals(results.length, 1);
			test.equals(results[0], '{{> 01-molecules/06-components/components-single-comment:foo }}');
		...
		'find_pattern_partials_with_style_modifiers finds no style modifiers when only partials present' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment }}</div>",
			};
		...
		'find_pattern_partials_with_style_modifiers finds no style modifiers when only partials with pattern parameters present' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment(foo: 'bar') }}</div>",
			};
		...
		'find_pattern_partials_with_parameters finds parameters' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> components-comment-header}}</h1><div>{{> components-single-comment(bar:'baz') }}</div>",
			};
			...
			test.equals(results.length, 1);
			test.equals(results[0], "{{> components-single-comment(bar:'baz') }}");
		...
		'find_pattern_partials_with_parameters finds parameters with verbose partials' : function(test){
			...
			//setup current pattern from what we would have during execution
			var currentPattern = {
				"template": "<h1>{{> 01-molecules/06-components/components-comment-header}}</h1><div>{{> 01-molecules/06-components/components-single-comment(bar:'baz') }}</div>",
			};
			...
			var results = pattern_assembler.find_pattern_partials_with_parameters(currentPattern);
			test.equals(results.length, 1);
			test.equals(results[0], "{{> 01-molecules/06-components/components-single-comment(bar:'baz') }}");
		...
		'setState - applies any patternState matching the pattern' : function(test){
			...
			patternlab.config.patternStates["features-homepage-emergency"] = "inprogress";

			var pattern = {
        key: "features-homepage-emergency"
			};
		...
		'get_pattern_by_key - returns the exact key if found' : function(test){
			...
			patternlab.patterns.push({
				key: 'components-primary-nav-jagged',
				subdir: 'molecules',
				fileName: 'primary-nav-jagged'
			}, {
				key: 'components-primary-nav',
				subdir: 'molecules',
				fileName: 'components-primary-nav'
			});

			//act
			var result = pattern_assembler.get_pattern_by_key('components-primary-nav', patternlab);
			//assert
